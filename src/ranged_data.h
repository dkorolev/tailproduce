...
namespace TailProduce {
    ...
    class RangedData : public std::enable_if<std::is_signed<RANGE_TYPE>::value> {
    public: 
        RangedData() = default;
        RANGE_TYPE DefaultValue() { return DefaultValue(); }
        std::string GetMin() { return asString_(std::numeric_limits<RANGE_TYPE>::min()); }
        std::string GetMax() { return asString_(std::numeric_limits<RANGE_TYPE>::max()); }
...
// Uint8_t are handled separatedly because they don't encode/decode to integer values.
    template <>
    class RangedData<uint8_t> {
    public: 
        RangedData() = default;
        uint8_t DefaultValue() { return DefaultValue(); }
        std::string GetMin() { return asString_(std::numeric_limits<uint8_t>::min()); }
        std::string GetMax() { return asString_(std::numeric_limits<uint8_t>::max()); }
...
// Take a time point and convert it to a uint64_t, not a time_t which is signed.
    template <>
    class RangedData <std::chrono::time_point<std::chrono::system_clock>> {
    public:
        RangedData() = default;
        std::chrono::time_point<std::chrono::system_clock> DefaultValue() { return std::chrono::system_clock::now(); }
        std::string GetMin() { return dr_.GetMin(); }
        std::string GetMax() { return dr_.GetMax(); }
        std::string GetValue(std::chrono::time_point<std::chrono::system_clock> val = std::chrono::system_clock::now()) { 
            return dr_.GetValue(std::chrono::system_clock::to_time_t(val)); }
        std::string GetValue(time_t val) { return dr_.GetValue(val); }
        std::string GetValue(uint64_t val) { return dr_.GetValue(val); }

        uint NDigits() { 
            return dr_.NDigits();
        }
        std::chrono::time_point<std::chrono::system_clock>
            Reverse(std::string const& component) {
