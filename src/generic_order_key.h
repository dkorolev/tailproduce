...
namespace TailProduce {
    template<typename PRIMARY_KEY, typename SECONDARY_KEY>
    class GenericOrderKey : public ::TailProduce::OrderKey {
    public:
        GenericOrderKey(std::string const& streamId,
                        ConfigValues const& cv) : config_(cv) {
        ...
        Storage::KEY_TYPE 
        CreateKey(decltype(std::declval<PRIMARY_KEY>().Reverse("")) pval) {
        ...
        decltype(std::declval<PRIMARY_KEY>().Reverse("")) 
        ReversePrimary(Storage::KEY_TYPE const& orderKey) {
            Storage::KEY_TYPE primaryKey = orderKey.substr(prefix_.length(), primary_.NDigits());
            return primary_.Reverse(primaryKey);
        }

        decltype(std::declval<SECONDARY_KEY>().Reverse("")) 
        ReverseSecondary(Storage::KEY_TYPE const& orderKey) {
